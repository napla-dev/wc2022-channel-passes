# Passes into the pocket — WC2022

**Detecting and ranking passes into the full-back / centre-back channel ("the pocket") from event data alone.**

The *pocket* is the channel between a defending full-back (or wing-back) and the
...
![Where pocket passes are received](outputs/figures/reception_map.png)
...
```
  ...
  event data     │  high-recall candidates   │      │  precision filter        │      pocket
                 │  (label-free, spatial)    │      │  (logistic regression)   │      passes
                 └───────────────────────────┘      └──────────────────────────┘
                        2,226 candidates                 scored 0–1 probability
```

### Stage 1 — spatial candidate generator (`src/pocket.py`)
...
### Stage 2 — supervised classifier (`scripts/pocket_classifier.py`)
...
- **moderate isolation beats extreme isolation** — a true pocket sits *between*
  two defenders; a reception in wide-open space is usually a through ball, not a
  channel entry,
- **a passer already past the defensive line is less likely** a true pocket
...
A pass is counted as a pocket pass when the Stage-2 probability ≥ 0.5; team
...
already in this repo (`outputs/pocket_passes_scored.csv`, a few hundred KB) — no
raw data needed:

```bash
pip install -r requirements.txt
python scripts/make_figures.py          # regenerate every figure
python scripts/pocket_classifier.py     # retrain + re-score (needs gold labels)
```
...
```
src/pocket.py              Stage 1 — spatial candidate generator
src/data_loader.py         PFF event/metadata loaders (event-only)
src/viz.py                 pitch drawing + plot style
scripts/extract_pockets.py run Stage 1 over all 64 matches
scripts/pocket_classifier.py  Stage 2 — train, cross-validate, score
...
- **Gold labels cover Argentina's 7 matches only.** They were produced by manual
  ...
  those games. The model's generalisation to other teams' styles is therefore
  **unvalidated** — the headline CV number should be read as indicative, not
  definitive.
- **Negatives are partly "weak".** Most negatives are candidates the reviewer
  saw but did not flag as pockets, rather than independently verified
...
flagged genuine pocket receptions with high precision. Stage 2 is a promising
